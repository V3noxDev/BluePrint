...
test('understands escaped separators generated by Minecraft', () => {
    const source = 'level-type=minecraft\\:normal\nmotd=Original\n';
    const document = parseProperties(source);

    assert.equal(propertiesToRecord(document)['level-type'], 'minecraft:normal');
    assert.equal(updateProperty(source, 'motd', 'Novo'), 'level-type=minecraft\\:normal\nmotd=Novo\n');
});
